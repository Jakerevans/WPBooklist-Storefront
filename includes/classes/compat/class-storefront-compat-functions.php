<?php
/**
 * Class StoreFront_Compat_Functions - class-storefront-compat-functions.php
 *
 * @author   Jake Evans
 * @category Admin
...
if ( ! class_exists( 'StoreFront_Compat_Functions', false ) ) :
	/**
	 * StoreFront_Compat_Functions class. Here we'll run functions that make older versions of WPBookList compatible with newest version
	 */
	class StoreFront_Compat_Functions {
		...
		public function __construct() {
			...
			$split_string     = explode( 'storefront', $row->extensionversions );
			$first_part       = $split_string[0];
			$last_part        = substr( $split_string[1], 5 );
			$this->new_string = $first_part . 'storefront' . WPBOOKLIST_STOREFRONT_VERSION_NUM . $last_part;
			...
			$this->storefront_update_version_number_function();
		 ...
		 */
		public function storefront_update_version_number_function() {
